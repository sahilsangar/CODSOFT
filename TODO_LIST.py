...
def add_list():  
    get_task = thr_label.get()  
    if len(get_task) == 0:   
       messagebox.showerror( 'Field is Empty!',"Please fill Task")  
    else:  
        type_task.append(get_task)          
        obj.execute('insert into type_task values (?)', (get_task ,))        
        update_list()           
...
def delete_list():     
    try:          
        add = listbox.get(listbox.curselection())            
        if add in type_task:             
            type_task.remove(add)              
            update_list()              
            obj.execute('delete from type_task where title = ?', (add,))  
    except:            
        messagebox.showerror('No Task Selected',"Please select Task")      
  
def delete_all_list():  
    all_task = messagebox.askyesno('Delete All', 'Are you sure?')      
    if all_task == True:  
...
def retrieve_database():  
    while(len(type_task) != 0):            
        type_task.pop()       
    for row in obj.execute('select title from type_task'):       
...
if __name__ == "__main__":      
    ...
    obj.execute('create table if not exists type_task (title text)')   
